...
check_database_config() {
    ...
    if [ -f "config/config.yaml" ]; then
        ...
        if command -v mysql &>/dev/null; then
            # Intentar obtener configuración de DB
            local DB_NAME=$(yq -r '.database.name' config/config.yaml 2>/dev/null || echo "")
            ...
            if [ ! -z "$DB_NAME" ]; then
                ...
                echo -e "  ${RED}${CROSS} Configuración de base de datos incompleta${RESET}"
            fi
        else
            echo -e "  ${YELLOW}${WARNING} No se puede verificar la base de datos (MySQL no disponible)${RESET}"
        fi
    else
